            ...
            onPressed: () async {
              ...
              try {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Error: Duplicate Roll Number?'),
                    ),
                  );
                }
              }
            ...
            child: _isLoading
                ...
                : TabBarView(
                    ...
                    children: _deptNames.map((dept) {
                      final tabStudents = _filteredStudents
                          .where((s) => s['departmentName'] == dept)
                          .toList();
                      ...
                      tabStudents.sort((a, b) {
                        ...
                        try {
                          return int.parse(
                            r1.replaceAll(RegExp(r'[^0-9]'), ''),
                          ).compareTo(
                            int.parse(r2.replaceAll(RegExp(r'[^0-9]'), '')),
                          );
                        } catch (e) {
                      ...
                      if (tabStudents.isEmpty) {
                        return Center(
                          child: Text(
                            "No students found in $dept matching '$_searchQuery'",
                          ),
                        );
                      }
