...
class _ManageEnrollmentScreenState extends State<ManageEnrollmentScreen> {
  ...
  Future<void> _loadInitialData() async {
    ...
    setState(() {
      ...
      if (_uniqueSubjects.isNotEmpty)
        _selectedSubjectName = _uniqueSubjects.first;
  ...
  Future<void> _fetchStudentsAndEnrollment() async {
    ...
    // 1. Get Students
    final students = await DatabaseHelper.instance.readStudentsByDept(
      _selectedDeptId!,
    );
    ...
    students.sort((a, b) {
      ...
      int aNum =
          int.tryParse(a.rollNumber.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
      int bNum =
          int.tryParse(b.rollNumber.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
  ...
  Future<void> _saveChanges() async {
    ...
    for (var lectureId in matchingLectureIds) {
      await DatabaseHelper.instance.updateBatchEnrollment(
        lectureId,
        toAdd,
        toRemove,
      );
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Updated $updatedCount batches of '$_selectedSubjectName'!",
          ),
              ...
              if (text.isNotEmpty) {
                ...
                setState(() {
                  for (var s in _students) {
                    ...
                    String rollDigits = s.rollNumber.replaceAll(
                      RegExp(r'[^0-9]'),
                      '',
                    );
                    if (rollDigits.isNotEmpty) {
                      ...
                      if (targetRolls.contains(rollNum) ||
                          targetRolls.contains(shortRoll)) {
