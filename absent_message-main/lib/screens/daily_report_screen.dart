...
class _DailyReportScreenState extends State<DailyReportScreen> {
  ...
  void _editLecture() async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => ReportGeneratorScreenV3(initialDate: _selectedDate),
      ),
    );
    _fetchData();
  }
  ...
  Future<void> _shareSummary() async {
    ...
    sb.writeln(
      "- Daily Turnout: ${(_dayAttendancePercent * 100).toStringAsFixed(1)}%\n",
    );
    ...
    for (var lecture in _lectureData) {
      ...
      sb.writeln(
        "Present: ${lecture.presentCount}/${lecture.totalStrength} | Absent: ${lecture.absentCount}",
      );
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final allStudents = await DatabaseHelper.instance
      ...
      Set<String> enrollments = enrollmentRows
      ...
      Set<int> uniqueScheduledStudents =
          {}; // Total unique students having classes
      ...
      for (var log in logs) {
        ...
        int deptId = 0;
        var matchingStudent = allStudents.firstWhere(
          (s) => s['departmentName'] == deptName,
          orElse: () => {},
        );
        ...
        List<String> absList = absenteesStr
            .split(',')
        ...
        for (var roll in lectureStudentRolls) {
          ...
          if (sId != null) {
            ...
            if (absList.contains(roll)) {
              uniqueAbsentStudents.add(
                sId,
              ); // Missed this class -> Add to Unique Absent
            } else {
              uniquePresentStudents.add(
                sId,
              ); // Attended this class -> Add to Unique Present
            }
          }
        }
        ...
        int lecPresent = (totalLecStrength - lecAbsent).clamp(
          0,
          totalLecStrength,
        );
        ...
        try {
          absentUiList.sort(
            ...
            ).compareTo(int.parse(b.rollNo.replaceAll(RegExp(r'[^0-9]'), ''))),
          );
        } catch (_) {}
      ...
      double dayPercent = totalSlotsScheduled == 0
          ? 0
          : (totalSlotsPresent / totalSlotsScheduled);

      if (mounted) {
        setState(() {
          // ✅ SYMMETRIC LOGIC:
          _totalAbsent = uniqueAbsentStudents
              .length; // Count of anyone who missed >= 1 class
          _totalPresent = uniquePresentStudents
              .length; // Count of anyone who attended >= 1 class
          ...
          subtitle: Padding(
            ...
            child: Row(
              children: [
                ...
                Text(
                  "Present: ${slot.presentCount} / ${slot.totalStrength}",
                  style: TextStyle(
                    color: Colors.green[700],
                    fontSize: 13,
